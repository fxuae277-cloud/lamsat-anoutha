 ...
 */
export async function pngToEscposRaster(pngBuffer, paperWidth, feedLinesAfter = 4, openDrawer = false) {
    ...
    if (width !== widthDots) {
        throw new Error(`unexpected sharp output width=${width} (expected ${widthDots})`);
    }
    if (channels !== 1) {
    ...
    for (let rowStart = 0; rowStart < height; rowStart += MAX_ROWS_PER_BLOCK) {
        ...
        out.push(bitmap.subarray(rowStart * bytesPerRow, (rowStart + rowsInBlock) * bytesPerRow));
        blocks++;
    }
    ...
    for (let i = 0; i < feedLinesAfter; i++)
        out.push(Buffer.from([LF]));
    if (openDrawer) {
...
//# sourceMappingURL=escposRaster.js.map