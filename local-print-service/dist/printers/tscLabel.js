...
const LOGO_BLOCK = { x: 116, y: 8, width: 240, height: 55 };
 ...
 */
export function buildVariantString(color, size) {
    const parts = [];
    const c = (color ?? "").trim();
    const s = (size ?? "").trim();
    if (c)
...
async function renderBarcodeSvg(barcode) {
...
export async function buildLabelSvg(input, hasLogo) {
...
// ─── PNG rasterisation ─────────────────────────────────────────────────────
export async function svgToLabelPng(svg, logo) {
 ...
 */
export async function pngToTspl(pngBuffer, copies) {
    ...
    if (width !== LABEL_WIDTH_PX || height !== LABEL_HEIGHT_PX) {
        throw new Error(`unexpected raster size ${width}×${height} (expected ${LABEL_WIDTH_PX}×${LABEL_HEIGHT_PX})`);
    }
    if (channels !== 1) {
    ...
    const header = Buffer.from(`SIZE ${LABEL_WIDTH_MM} mm,${LABEL_HEIGHT_MM} mm\r\n` +
        `GAP ${LABEL_GAP_MM} mm,0\r\n` +
        `DIRECTION 1\r\n` +
        `DENSITY 8\r\n` +
        `SPEED 4\r\n` +
        `CLS\r\n` +
        `BITMAP 0,0,${bytesPerRow},${height},0,`, "ascii");
 ...
 */
export async function printLabel(printerName, input) {
    const variantPreview = (input.productVariant ?? "").trim()
        || buildVariantString(input.color, input.size)
        || "(none)";
    console.log(`[Label] request productName="${input.productName}" ` +
    ...
    const timeoutPromise = new Promise((_, reject) => {
        setTimeout(() => reject(Object.assign(new Error("printer timeout"), { code: "ETIMEDOUT" })), 15_000);
    });
    await Promise.race([printPromise, timeoutPromise]);
    console.log(`[Label] sent to printer "${printerName}"`);
    return { pngBytes: png.length, tsplBytes: tspl.length };
}
//# sourceMappingURL=tscLabel.js.map