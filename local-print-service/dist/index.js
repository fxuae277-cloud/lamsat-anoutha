...
const LABEL_PRINTER_NAME = (process.env.LABEL_PRINTER_NAME || "TSC TTP-244M Pro").trim();
...
const handleStructuredLabelPrint = async (req, res) => {
    ...
    const productVariant = typeof body.productVariant === "string" ? body.productVariant.trim() : "";
    const color = typeof body.color === "string" ? body.color.trim() : "";
    const size = typeof body.size === "string" ? body.size.trim() : "";
    const printerName = typeof body.printerName === "string" && body.printerName.trim().length > 0
...
const handleLabelPrint = async (req, res) => {
    ...
    const { printerName, imageBase64, labelSize, columns, items, } = body;
    if (!printerName || typeof printerName !== "string") {
        res.status(400).json({ ok: false, error: "printerName is required" });
        return;
    }
