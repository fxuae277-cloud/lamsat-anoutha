...
function build80(invoice) {
    ...
    const oneLine = `${branchLabel} ${branchVal}` + "    " + `${cashierLabel} ${cashierVal}`;
    if (oneLine.length <= COLS) {
        b.textln(pad(`${branchLabel} ${branchVal}`, `${cashierLabel} ${cashierVal}`));
    }
    else {
    ...
    b.textln(pad("Discount:", invoice.discount > 0 ? `-${omr(invoice.discount)}` : omr(0)));
...
//# sourceMappingURL=printInvoice.js.map