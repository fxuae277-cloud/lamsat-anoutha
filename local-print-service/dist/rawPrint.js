 ...
 */
export async function printText(printerName, text) {
    if (!printerName?.trim())
    ...
    try {
        ...
        await execFileP("powershell.exe", ["-NoProfile", "-NonInteractive", "-Command", cmd], { windowsHide: true });
    }
    finally {
 ...
 */
export async function printRawBytes(printerName, bytes) {
    if (!printerName?.trim())
        throw new Error("printerName is required");
    ...
    try {
        await execFileP("powershell.exe", ["-NoProfile", "-NonInteractive", "-EncodedCommand", encoded], { windowsHide: true, maxBuffer: 4 * 1024 * 1024 });
    }
    finally {
        unlink(tmpFile).catch(() => { });
    }
