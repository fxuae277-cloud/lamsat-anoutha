                ...
                <TableRow key={u.id} className={`hover:bg-pink-50/30 ${!u.isActive ? "opacity-60" : ""}`}>
                  ...
                  <TableCell>
                    <div className="flex items-center gap-1">
                      <Button size="icon" variant="ghost" className="h-8 w-8" onClick={() => openEdit(u)} title={t("common.edit")}>
                        <Pencil className="h-3.5 w-3.5" />
                      </Button>
                      <Button size="icon" variant="ghost" className="h-8 w-8" onClick={() => { setSelectedUser(u); setResetOpen(true); }} title={t("users_mgmt.reset_pw_title")}>
                        <KeyRound className="h-3.5 w-3.5" />
                      </Button>
                      <Button size="icon" variant="ghost" className="h-8 w-8" onClick={() => toggleMut.mutate(u.id)} title={u.isActive ? t("users_mgmt.btn_deactivate") : t("users_mgmt.btn_activate")}>
                        {u.isActive ? <ToggleRight className="h-3.5 w-3.5 text-green-600" /> : <ToggleLeft className="h-3.5 w-3.5 text-gray-400" />}
                      </Button>
                      {me?.id !== u.id && (
                        <Button size="icon" variant="ghost" className="h-8 w-8 text-red-500 hover:text-red-700" onClick={() => { setSelectedUser(u); setDeleteOpen(true); }} title={t("common.delete")}>
