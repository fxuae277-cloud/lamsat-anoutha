...
const ACTION_LABELS: Record<string, { label: string; color: string; icon: any }> = {
  sale_return: { label: "مرتجع مبيعات", color: "bg-red-100 text-red-700 border-red-200", icon: RotateCcw },
  order_cancel: { label: "إلغاء طلب", color: "bg-amber-100 text-amber-700 border-amber-200", icon: AlertTriangle },
  order_status_change: { label: "تغيير حالة طلب", color: "bg-blue-100 text-blue-700 border-blue-200", icon: ShoppingCart },
  user_update: { label: "تعديل مستخدم", color: "bg-purple-100 text-purple-700 border-purple-200", icon: User },
  password_reset: { label: "إعادة تعيين كلمة مرور", color: "bg-orange-100 text-orange-700 border-orange-200", icon: Shield },
  price_change: { label: "تغيير سعر", color: "bg-indigo-100 text-indigo-700 border-indigo-200", icon: FileText },
};

const ENTITY_TYPES = [
  { value: "all", label: "الكل" },
  { value: "sale_return", label: "مرتجعات" },
  ...
  if (!isOwnerAdmin) {
    return (
      <div className="p-8 text-center text-muted-foreground">
        ...
        <p className="text-sm mt-1">سجل المراجعة متاح فقط للمالك والمدير</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300">
      <div>
        ...
        <p className="text-muted-foreground mt-1">تتبع كل العمليات الحساسة: الإلغاء، المرتجعات، تعديل البيانات</p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <Card>
          <CardContent className="p-4 flex items-center gap-3">
            ...
            <div>
              <p className="text-xs text-muted-foreground">تغييرات حالة</p>
      ...
      <div className="bg-card border shadow-sm rounded-xl overflow-hidden">
        <div className="p-4 border-b flex flex-wrap items-center gap-3 bg-muted/20">
          ...
          <div className="space-y-1 flex-1 min-w-[200px]">
            <label className="text-xs font-medium">بحث</label>
